...
import { STRONG_PASSWORD_PATTERN } from "./companyRegistrationSchema";

// Stores change password validation schema for the steps below.
export const changePasswordValidationSchema: {
  currentPassword: RegisterOptions<ChangePasswordRequest, "currentPassword">;
  newPassword: RegisterOptions<ChangePasswordRequest, "newPassword">;
} = {
  currentPassword: {
...
): RegisterOptions<ChangePasswordRequest, "newPassword"> => ({
...
export const createChangePasswordConfirmationValidation = (
  newPassword: string,
): RegisterOptions<ChangePasswordRequest, "confirmPassword"> => ({
  required: "Confirm password is required.",

  validate: (value) =>
    value === newPassword || "New password and confirm password must match.",
});
