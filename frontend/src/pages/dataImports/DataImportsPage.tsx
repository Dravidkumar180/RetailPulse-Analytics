...
export default function DataImportsPage() {
  ...
  const upload = useMutation({ mutationFn: () => uploadImport(type, file!), onSuccess: value => { setJob(value); setApiError(""); client.invalidateQueries({queryKey:["import-history"]}); refreshActivity(); }, onError: error => setApiError(errorMessage(error)) });
  const process = useMutation({ mutationFn: () => processImport(job!.id), onSuccess: value => { setJob(value); setApiError(""); client.invalidateQueries({queryKey:["import-history"]}); client.invalidateQueries({queryKey:["products"]}); client.invalidateQueries({queryKey:["customers"]}); client.invalidateQueries({queryKey:["sales"]}); refreshActivity(); }, onError: error => { setApiError(errorMessage(error)); client.invalidateQueries({queryKey:["import-history"]}); refreshActivity(); } });
  ...
  const clear = () => { setFile(null); setJob(null); setFileError(""); setApiError(""); if (input.current) input.current.value = ""; };
  ...
  return <Box className="imports-page">
    ...
    <section className="imports-card import-types"><div className="imports-section-head"><div><span>Step 1</span><h2>Select import type</h2></div><p>Choose the data your CSV contains.</p></div>
    ...
    <div className="imports-workspace">
      <section className="imports-card upload-card"><div className="imports-section-head"><div><span>Step 2</span><h2>Upload CSV file</h2></div></div>
        ...
        <Button variant="contained" disabled={!file || upload.isPending || !!fileError} onClick={()=>upload.mutate()}>{upload.isPending ? "Uploading & validating…" : "Upload & validate"}</Button>
      </section>
    ...
    {job && <>
      <section className="imports-card preview-card"><div className="imports-section-head"><div><span>Step 3</span><h2>Preview · {job.filename}</h2></div><p>{job.totalRecords.toLocaleString()} total records · {job.columns.length} detected columns</p></div>
      ...
      <section className={`imports-card result-card ${finished ? "complete" : ""}`}><div className="result-icon">{finished ? <CheckCircleOutlineIcon/> : <CheckCircleOutlineIcon/>}</div><div><span>{finished ? "Import result" : "Step 4"}</span><h2>{finished ? job.status : "Ready to import"}</h2><p>{finished ? `${job.successfulRecords} records added successfully. ${job.failedRecords} invalid and ${job.duplicateRecords} duplicate records were skipped.` : `${job.validRecords} valid records are ready. Invalid and duplicate rows will be skipped safely.`}</p></div><div className="result-actions">{!finished && <Button variant="contained" disabled={!job.validRecords || process.isPending} onClick={()=>process.mutate()}>Import {job.validRecords} valid records</Button>}{(job.failedRecords+job.duplicateRecords)>0 && <Button variant="outlined" startIcon={<DownloadOutlinedIcon/>} onClick={()=>downloadImportErrors(job)}>Download failed records</Button>}</div></section>
    </>}
    <section className="imports-card history-card"><div className="imports-section-head"><div><span>Import log</span><h2>Import history</h2></div><p>Only imports for your company are shown.</p></div>
