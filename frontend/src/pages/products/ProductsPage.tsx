...
import { Box, Drawer, IconButton, MenuItem, Pagination, TextField, Typography } from "@mui/material";
...
const ProductsPage=()=>{
 ...
 const qc=useQueryClient(); const [search,setSearch]=useState(""); const [category,setCategory]=useState(""); const [status,setStatus]=useState(""); const [brand,setBrand]=useState(""); const [sort,setSort]=useState("recent"); const [page,setPage]=useState(1); const [editing,setEditing]=useState<Product|null>(null); const [form,setForm]=useState<ProductInput>(empty); const [open,setOpen]=useState(false); const [error,setError]=useState("");
 ...
 const pageSize=5; const filteredProducts=products.data?.items||[]; const pageCount=Math.max(1,Math.ceil(filteredProducts.length/pageSize)); const visibleProducts=filteredProducts.slice((page-1)*pageSize,page*pageSize);
