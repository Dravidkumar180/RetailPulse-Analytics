...
export const VALIDATION_MESSAGES = {
  ...
  minimumLength: (fieldName: string, minimumLength: number): string =>
    `${fieldName} must contain at least ${minimumLength} characters.`,

  maximumLength: (fieldName: string, maximumLength: number): string =>
  ...
  weakPassword: "Use uppercase, lowercase, number and special character.",

  passwordMismatch: "Password and confirm password must match.",

  newPasswordMismatch: "New password and confirm password must match.",

  samePassword: "New password must be different from the current password.",

  companyAlreadyExists: "A company with this name or email already exists.",
