...
export const getCategories = async (search = "") =>
  (
    await axiosInstance.get<{ items: Category[]; total: number }>(
      "/categories",
...
export const createCategory = async (data: CategoryInput) =>
  (await axiosInstance.post<Category>("/categories", data)).data;
// Saves category.
export const updateCategory = async (id: string, data: CategoryInput) =>
  (await axiosInstance.put<Category>(`/categories/${id}`, data)).data;
// Removes category.
export const deleteCategory = async (id: string) => {
  await axiosInstance.delete(`/categories/${id}`);
};
// Gets products.
export const getProducts = async (params: Record<string, string | undefined>) =>
  (await axiosInstance.get<ProductList>("/products", { params })).data;
// Adds product.
export const createProduct = async (data: ProductInput) =>
  (await axiosInstance.post<Product>("/products", data)).data;
// Saves product.
export const updateProduct = async (id: string, data: ProductInput) =>
  (await axiosInstance.put<Product>(`/products/${id}`, data)).data;
// Removes product.
