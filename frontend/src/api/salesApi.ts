...
// Gets sales.
export const getSales = async (params: SalesFilters = {}) =>
  (
    await axiosInstance.get<{ items: Sale[]; total: number }>("/sales", {
      params,
    })
  ).data;
// Gets sales summary.
export const getSalesSummary = async () =>
  (await axiosInstance.get<SalesSummary>("/sales/summary")).data;
// Adds sale.
export const createSale = async (data: SaleInput) =>
  (await axiosInstance.post<Sale>("/sales", data)).data;
// Saves sale.
export const updateSale = async (id: string, data: SaleInput) =>
  (await axiosInstance.put<Sale>(`/sales/${id}`, data)).data;
// Removes sale.
export const deleteSale = async (id: string) => {
  await axiosInstance.delete(`/sales/${id}`);
};
export const logSaleExport = async (id: string, report: "pdf" | "csv") => {
  await axiosInstance.post(`/sales/${id}/export`, null, { params: { report } });
};
export const logSalesReportExport = async () => {
  await axiosInstance.post("/sales/report/export");
};
