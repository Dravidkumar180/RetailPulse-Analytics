...
export interface SaleInput { customerId:string; customerName:string; saleDate:string; salesChannel:SalesChannel; paymentMethod:PaymentMethod; paymentStatus:PaymentStatus; notes?:string; items:SaleItemInput[]; }
...
export interface Sale { id:string; invoiceNumber:string; customerId?:string; customerName:string; saleDate:string; salesChannel:SalesChannel; paymentMethod:PaymentMethod; paymentStatus:PaymentStatus; notes?:string; subtotal:number; discount:number; tax:number; totalAmount:number; createdByName:string; items:SaleItem[]; createdAt:string; updatedAt:string; inventoryAlerts:string[]; }
...
export interface SalesFilters { search?:string; startDate?:string; endDate?:string; categoryId?:string; salesChannel?:string; paymentMethod?:string; paymentStatus?:string; sort?:string; }
...
export const logSaleExport = async (id:string, report:"pdf"|"csv") => { await axiosInstance.post(`/sales/${id}/export`, null, { params: { report } }); };
export const logSalesReportExport = async () => { await axiosInstance.post("/sales/report/export"); };
