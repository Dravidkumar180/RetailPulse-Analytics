/* Teaching guide: This file contains analytics api API requests, response types, and data mapping.
...
export const getAnalyticsDashboard = async (
  ...
  const raw = (
    await axiosInstance.get<Record<string, any>>("/analytics/dashboard", {
      params,
    })
  ).data;
  const kpis = raw.kpis || {},
  ...
  if (!topCustomers.length) {
    try {
      const [customerAnalytics, customerList] = await Promise.all([
        getCustomerAnalytics(),
        getCustomers({ status: "ACTIVE", pageSize: "100" }),
      ]);
      topCustomers = (customerAnalytics.topCustomers || [])
        .map((ranked, index) => {
          const customer = customerList.items.find(
            (item) => item.fullName === ranked.name,
          );
          ...
          return {
            ...
            totalSpend: spend,
            averageOrderValue:
              Number(customer?.summary.averageOrderValue) ||
  ...
  return {
    ...
    topProducts: Array.isArray(raw.topProducts)
      ? raw.topProducts.map((x: any, index: number) => ({
          id: String(x.id || x.productId || index),
          name: String(x.name || "Unknown product"),
          sku: String(x.sku || "—"),
    ...
    options: {
      ...
      paymentStatuses: options.paymentStatuses || ["PAID", "PENDING", "FAILED"],
    },
    lastUpdated: raw.lastUpdated || new Date().toISOString(),
  };
};
export const logAnalyticsAction = async (
